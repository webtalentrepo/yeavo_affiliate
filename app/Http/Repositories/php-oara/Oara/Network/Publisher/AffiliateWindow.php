...
private $_timeZone = "Europe/Berlin";
    ...
    public function login($credentials)
    {
        ...
        $this->_exportClient = new Access($credentials);
    }
    ...
    public function getMerchantList()
    {
        ...
        try {
            ...
            foreach ($a_status as $status) {
                ...
                $merchantList = [];
                $urls = [];
                $urls[] = new Request('https://api.awin.com/publishers/' . $id . '/programmes/?relationship=' . $status . '&accessToken=' . $pwd, []);
    ...
    public function getVouchers($apiKey)
    {
        ...
        try {
            ...
            $id = $this->_credentials["accountid"];
            $params = [
                new Parameter('promotionType', 'voucher'),
                new Parameter('categoryIds', ''),
                new Parameter('regionIds', ''),
                new Parameter('advertiserIds', ''),
                new Parameter('membershipStatus', ''),
                new Parameter('promotionStatus', 'active'),
            ];

            $urls[] = new Request('https://ui.awin.com/export-promotions/' . $id . '/' . $apiKey . '?', $params);
            $result = $this->_exportClient->get($urls);
            if ($result === false || !is_array($result)) {
            ...
            if ($result === false) {
                ...
                foreach ($transactionObjectFull as $transactionObject) {
                    ...
                    if (is_object($transactionObject->clickRefs)) {
                        ...
                        elseif (property_exists($transactionObject->clickRefs, 'clickRef2') && $transactionObject->clickRefs->clickRef2 != null && $transactionObject->clickRefs->clickRef2 != 0)
                            $transaction['custom_id'] = $transactionObject->clickRefs->clickRef2;
                        elseif (property_exists($transactionObject->clickRefs, 'clickRef3') && $transactionObject->clickRefs->clickRef3 != null && $transactionObject->clickRefs->clickRef3 != 0)
                            $transaction['custom_id'] = $transactionObject->clickRefs->clickRef3;
                        elseif (property_exists($transactionObject->clickRefs, 'clickRef4') && $transactionObject->clickRefs->clickRef4 != null && $transactionObject->clickRefs->clickRef4 != 0)
                            $transaction['custom_id'] = $transactionObject->clickRefs->clickRef4;
                        elseif (property_exists($transactionObject->clickRefs, 'clickRef5') && $transactionObject->clickRefs->clickRef5 != null && $transactionObject->clickRefs->clickRef5 != 0)
                            $transaction['custom_id'] = $transactionObject->clickRefs->clickRef5;
                        elseif (property_exists($transactionObject->clickRefs, 'clickRef6') && $transactionObject->clickRefs->clickRef6 != null && $transactionObject->clickRefs->clickRef6 != 0)
                    ...
                    if ($transactionObject->commissionStatus == 'approved') {
                        ...
                        $transaction['status'] = Utilities::STATUS_PENDING;
                    } elseif ($transactionObject->commissionStatus == 'declined') {
                        $transaction['status'] = Utilities::STATUS_DECLINED;
                    }
                    $transaction['amount'] = Utilities::parseDouble($transactionObject->saleAmount->amount);
                    $transaction['commission'] = Utilities::parseDouble($transactionObject->commissionAmount->amount);
    ...
    public function getPaymentHistory()
    {
        ...
        $urls = [];
        $urls[] = new Request("https://darwin.affiliatewindow.com/awin/affiliate/" . $this->_userId . "/payments/history?", []);
        ...
        while (!$finished) {
            foreach ($results as $result) {
                ...
                if ($linkList->length > 0) {
                    ...
                    $obj['value'] = Utilities::parseDouble($linkList->item(3)->nodeValue);
            ...
            if ($results->length > 0) {
                foreach ($results as $nextPageLink) {
                    ...
                    $urls = [];
                    $urls[] = new Request("https://darwin.affiliatewindow.com" . $nextPageUrl, []);
                    $exportReport = $this->_exportClient->get($urls);
                    $doc = new DOMDocument();
                    @$doc->loadHTML($exportReport[0]);
                    $xpath = new DOMXPath($doc);
    ...
    public function paymentTransactions($paymentId)
    {
        $transactionList = [];
        $urls = [];
        $urls[] = new Request("https://darwin.affiliatewindow.com/awin/affiliate/" . $this->_userId . "/payments/download/paymentId/" . $paymentId, []);
        $exportReport = $this->_exportClient->get($urls);
    ...
    public function getAdvertisers($apiKey)
    {
        ...
        try {
            $urls[] = new Request('https://productdata.awin.com/datafeed/list/apikey/' . $apiKey, []);
            $result = $this->_exportClient->get($urls);
            if ($result === false || !is_array($result)) {
