...
class Digidip extends Network
{
        ...
        try {
            ...
            foreach ($a_dates as $dates) {
                ...
                while ($loop) {
                    ...
                    $params = [
                        new Parameter('project_id', $id_site),
                        new Parameter('timestamp_start', $dStartDate->getTimestamp()),
                        new Parameter('timestamp_end', $dEndDate->getTimestamp()),
                    ...
                    curl_setopt($ch, CURLOPT_HTTPHEADER, ["Authorization: Basic " . base64_encode($user . ':' . $password), "Accept: application/json"]);
    ...
    public function createTransactionArray($transactionJson)
    {
        ...
        $transaction['amount'] = Utilities::parseDouble($transactionJson['price']['amount']);
        $transaction['commission'] = Utilities::parseDouble($transactionJson['commission']['amount']);
    ...
    public function getTransactions($merchantList = null, DateTime $dStartDate = null, DateTime $dEndDate = null)
    {
        ...
        while ($loop) {
            ...
            $params = [
                new Parameter('project_id', $id_site),
                new Parameter('timestamp_start', $dStartDate->timestamp),
                new Parameter('timestamp_end', $dEndDate->timestamp),
                new Parameter('page', $page),
            ];
