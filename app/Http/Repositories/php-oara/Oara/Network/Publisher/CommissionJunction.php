 ...
 *
 */
class CommissionJunction extends Network
{
    ...
    public function getTransactionList($merchantList = null, DateTime $dStartDate = null, DateTime $dEndDate = null)
    {
        ...
        if (!is_null($merchantList) && is_array($merchantList) && count($merchantList) > 0) {
            $merchantIdArray = array_keys(Utilities::getMerchantIdMapFromMerchantList($merchantList));
            ...
            foreach ($xml->commissions->commission as $singleTransaction) {
                ...
                if (count($this->_sitesAllowed) == 0 || in_array(( int )self::findAttribute($singleTransaction, 'website-id'), $this->_sitesAllowed)) {
                    ...
                    if (count($merchantIdList) == 0 || isset($merchantIdList[(int)self::findAttribute($singleTransaction, 'cid')])) {
                        ...
                        $transaction ['amount'] = Utilities::parseDouble(self::findAttribute($singleTransaction, 'sale-amount'));
                        $transaction ['commission'] = Utilities::parseDouble(self::findAttribute($singleTransaction, 'commission-amount'));
                        ...
                        } elseif (self::findAttribute($singleTransaction, 'action-status') == 'extended' || self::findAttribute($singleTransaction, 'action-status') == 'new') {
                            $transaction ['status'] = Utilities::STATUS_PENDING;
                        } elseif (self::findAttribute($singleTransaction, 'action-status') == 'corrected') {
    ...
    {
        $totalTransactions = [];
        $typeTransactions = ["bonus", "click", "impression", "sale", "lead", "advanced%20sale", "advanced%20lead", "performance%20incentive"];
