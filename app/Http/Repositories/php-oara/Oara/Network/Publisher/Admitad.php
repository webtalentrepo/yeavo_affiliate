...
class Admitad extends Network
{
    ...
    public function getToken()
    {
        ...
        $params = [
            new Parameter('grant_type', $this->_grant_type),
            new Parameter('client_id', $this->_client_id),
            new Parameter('scope', $this->_scope)
        ];
    ...
    public function getMerchantList()
    {
        ...
        try {
            ...
            while ($loop) {
                ...
                curl_setopt($ch, CURLOPT_RETURNTRANSFER, TRUE);
                curl_setopt($ch, CURLOPT_HTTPHEADER, ["Authorization: Bearer " . $this->getToken(), "Content-Type: application/x-www-form-urlencoded"]);
    ...
    public function getTransactionList($merchantList = null, DateTime $dStartDate = null, DateTime $dEndDate = null)
    {
        ...
        try {
            ...
            while ($loop) {
                ...
                $params = [
                    new Parameter('date_start', $dStartDate->format("d.m.Y")),
                    new Parameter('date_end', $dEndDate->format("d.m.Y")),
                ...
                foreach ($transactionsList['results'] as $transactionJson) {
                    ...
                    $transaction['amount'] = Utilities::parseDouble($transactionJson["cart"]);
                    $transaction['commission'] = Utilities::parseDouble($transactionJson["payment"]);
                    ...
                    } elseif ($transactionJson['status'] == 'declined') {
                        $transaction['status'] = Utilities::STATUS_DECLINED;
                    } elseif ($transactionJson['status'] == 'confirmed' || $transactionJson['status'] == 'approved') {
            ...
            while ($loop) {
                ...
                foreach ($coupons["results"] as $couponJson) {
                    ...
                    switch ($couponJson["species"]) {
                        case 'promocode':
                            $obj['type'] = Utilities::OFFER_TYPE_VOUCHER;
                            break;
                        case 'action':
                            $obj['type'] = Utilities::OFFER_TYPE_DISCOUNT;
                            break;
                        default:
                            $obj['type'] = Utilities::OFFER_TYPE_DISCOUNT;
