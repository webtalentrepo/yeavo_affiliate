 ...
 *
 */
class NetAffiliation extends Network
{
    ...
    public function checkConnection()
    {
         ...
         */
        $valuesFormExport = [];
        $valuesFormExport[] = new Parameter('authl', $this->_credentials["user"]);
        $valuesFormExport[] = new Parameter('authv', $this->_credentials["apiPassword"]);
        $urls = [];
        $urls[] = new Request('https://stat.netaffiliation.com/requete.php?', $valuesFormExport);
    ...
    public function getTransactionList($merchantList = null, DateTime $dStartDate = null, DateTime $dEndDate = null)
    {
        ...
        $valuesFormExport[] = new Parameter('authl', $this->_credentials["user"]);
        $valuesFormExport[] = new Parameter('authv', $this->_credentials["apiPassword"]);
        $valuesFormExport[] = new Parameter('champs', 'idprogramme,date,etat,argann,montant,gains,monnaie,idsite');
        $valuesFormExport[] = new Parameter('debut', $dStartDate->format("Y-m-d"));
        $valuesFormExport[] = new Parameter('fin', $dEndDate->format("Y-m-d"));
        $urls = [];
        $urls[] = new Request('https://stat.netaffiliation.com/requete.php?', $valuesFormExport);
        $exportReport = $this->_client->get($urls);
        ...
        for ($i = 1; $i < $num; $i++) {
            ...
            if (count($this->_sitesAllowed) == 0 || in_array($transactionExportArray[7], $this->_sitesAllowed)) {
                if (isset($merchantIdList[$transactionExportArray[0]])) {
                    ...
                    } elseif (strstr($transactionExportArray[2], 'r')) {
                        $transaction['status'] = Utilities::STATUS_DECLINED;
                    } elseif (strstr($transactionExportArray[2], 'a')) {
