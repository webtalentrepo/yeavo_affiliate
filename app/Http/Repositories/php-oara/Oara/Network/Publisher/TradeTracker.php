...
class TradeTracker extends Network
{
    ...
    public function login($credentials)
    {
        ...
        //Setting the client.
        $this->_apiClient = new SoapClient($wsdlUrl, [
            'encoding'     => 'UTF-8',
            ...
            foreach ($this->_apiClient->getConversionTransactions($affiliateSite->ID, $options) as $transaction) {
                if ($merchantList == null || isset($merchantIdList[(int)$transaction->campaign->ID])) {
                    ...
                    switch ($transaction->transactionStatus) {
                        case 'accepted':
                            $object['status'] = Utilities::STATUS_CONFIRMED;
                            break;
                        case 'pending':
                            $object['status'] = Utilities::STATUS_PENDING;
                            break;
                        case 'rejected':
                            $object['status'] = Utilities::STATUS_DECLINED;
                            break;
                        default:
                            $object['status'] = null;
                            break;
                    ...
                    $object['amount'] = Utilities::parseDouble($transaction->orderAmount);
                    $object['commission'] = Utilities::parseDouble($transaction->commission);
    ...
    public function getPaymentHistory()
    {
        ...
        foreach ($this->_apiClient->getPayments($options) as $payment) {
            ...
            $obj['method'] = 'BACS';
            $obj['value'] = Utilities::parseDouble($payment->endTotal);
            $paymentHistory[] = $obj;
        }
